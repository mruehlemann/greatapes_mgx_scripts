...
para.D = readRDS(paste0(this, ".user_msa.matrix.Rds"))
...
tree_bionj=read.tree(paste0("trees/",this, ".aln.bionj.bs.nwk"))
...
host_tree = mad(read.nexus("/work_ifs/sukmb276/Metagenomes/projects/ApesComplete/output/220616_analysis_final/cospec_analysis/host_trees_10kTrees/consensusTree_10kTrees_Primates_Version3_phylo.nex"),output_mode="full")[[6]][[1]]
...
output_hommola = data.frame(nperm_hommola=nperm_hommola, p_hommola=round(p_final_hommola,5))

write.table(output_hommola, paste0(this,".cospec_phylo.out"), row.names=F, sep="\t")
...
write.table(output_parafit, paste0(this,".cospec_phylo.out"), row.names=F, sep="\t")
