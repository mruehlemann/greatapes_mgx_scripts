...
panel_d_tree = panel_d_tree + geom_tippoint(aes(col=host, shape=host), size=5) + scale_color_manual(values=colset$col, breaks=colset$host) +
...
fig1_panel_d=plot_grid(plot_grid(panel_d_tree) + theme(plot.margin = unit(c(0,0,0,.5),"cm")), panel_d_heatmap+ theme(plot.margin = unit(c(0,0,0,0),"cm")), NULL, panel_d_bottom + theme(plot.margin = unit(c(0,0,0,0),"cm") ), rel_widths=c(.2,.8), rel_heights = c(.95,.1)) 
...
fig1_legend = get_legend(fig1_panel_b + guides(colour = guide_legend(ncol = 1, title="",byrow = TRUE), shape=guide_legend(ncol = 1, title="",byrow = TRUE)) + theme(legend.spacing.y = unit(0.1, 'cm') ,legend.position = "bottom", legend.justification = "center", legend.text = element_text(size=10), legend.key.size = unit(0.2, "cm")))

fig1_new=cowplot::plot_grid(
    cowplot::plot_grid(fig1_panel_a, fig1_panel_b +theme(legend.position = "none"), fig1_legend, ncol=3, labels=c("a","b"), rel_widths = c(.3,.4,.3)),
    cowplot::plot_grid(fig1_panel_c, fig1_panel_d, ncol=2, labels=c("c","d"), rel_widths=c(.45,.55)),
    ncol=1
    )
