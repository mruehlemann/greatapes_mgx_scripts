...
pp=pp +  theme(legend.position = "none") + ggtitle(paste0(pp_tax_cons, " spp."))
...
pp2=pp2+geom_point(data=pp2$data %>% filter(isTip==T), aes(x=x,y=y, shape=host, label=label, col=host, bin_id=GreatApes_95_final), size=3)+
  scale_color_manual(values=colset2$col, breaks=colset2$host)+
  scale_shape_manual(values=c(15,16,17,18,19)[colset2$shape],breaks=colset2$host) + coord_flip()
pp2=pp2 +  theme(legend.position = "none") + ggtitle(paste0(pp2_tax_cons, " spp."))
...
pp3=pp3+geom_point(data=pp3$data %>% filter(isTip==T), aes(x=x,y=y, shape=host, label=label, col=host, bin_id=GreatApes_95_final), size=3)+
  scale_color_manual(values=colset2$col, breaks=colset2$host)+
  scale_shape_manual(values=c(15,16,17,18,19)[colset2$shape],breaks=colset2$host) + coord_flip()
pp3=pp3 +  theme(legend.position = "none") + ggtitle(paste0(pp3_tax_cons, " spp."))
...
pp4=pp4+geom_point(data=pp4$data %>% filter(isTip==T), aes(x=x,y=y, shape=host, label=label, col=host, bin_id=GreatApes_95_final), size=3)+
  scale_color_manual(values=colset2$col, breaks=colset2$host)+
  scale_shape_manual(values=c(15,16,17,18,19)[colset2$shape],breaks=colset2$host) + coord_flip()
pp4=pp4 +  theme(legend.position = "none") + ggtitle(paste0(pp4_tax_cons, " spp."))
...
tree_legend= colset2 %>% arrange(host) %>% mutate(column = case_when(grepl("^G",host) ~ 1, grepl("^H", host) ~ 1, grepl("^P", host) ~ 2)) %>% group_by(column) %>% mutate(row=seq_along(column)) %>% 
  ...
  geom_point(aes(col=host_long, shape=host_long), size=3) + 
  geom_text(aes(label=host_long), hjust=0, nudge_x = .1, size=3) + 
...
cophy_tree_set = plot_grid(NULL, pp,pp2,pp3,pp4,NULL,tree_legend, NULL , rel_heights = c(0.05,0.2,0.2,0.2,0.2,0.025,0.1,0.025), ncol=1)
...
cophy_enrich_plot = cophy_stats2 %>% filter(tot>=10) %>% 
  ...
  geom_text(aes(label=stars), nudge_y = 5, col="black") + 
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1, size=9), axis.title = element_text(size=12), legend.position = "none") + 
...
coph_plot =  sgb_long %>% filter(variable %in% sgb_in_cospec) %>% mutate(pres = value > 0, cosp = variable %in% sgb_with_cophy) %>% filter(pres) %>% group_by(sample, host_long) %>% summarise(cophy_rat = mean(cosp)) %>% 
  ggplot(aes(x=host_long,y=cophy_rat*100, fill=host_long)) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(height=0, width=.2) +
  scale_fill_manual(values=colset$col, breaks = colset$host_long) + 
  theme(legend.position = "none", axis.title = element_text(size=12), axis.text.x = element_text(angle=45, hjust=1, size=9)) +
  ylab("Prop. of Co-Phylo SGBs (%)") + xlab("") + scale_y_continuous(expand=c(.01,0))
...
write.table(phylo_reg_results, "figures_and_tables/Suppl_Table_S16_Cophylo_Traits.tsv", sep="\t", row.names = F)
  ...
  geom_point(aes(col=case_when(q>=0.05 ~ "A", Estimate < 0 ~ "B", Estimate > 0 ~ "C")), size=2) + scale_x_continuous(limits=c(-1.7,1.7)) + 
  ggrepel::geom_text_repel(data= . %>% filter(q<0.05), aes(label=trait), min.segment.length = .1)  +
  ylab("-log(P)") + scale_color_manual(values=c("grey","#f12400","#3d9ab2")) + theme(legend.position = "none", axis.title = element_text(size=12)) + xlab("Effect size")
...
gs_plot = plot_dat_gs %>%  ggplot(aes(x=plot_pos, y=rel_genome_size, fill=cophy, group=paste(phylum,cophy) )) + geom_boxplot(outlier.shape = NA, position = position_dodge()) +  geom_jitter(width=.05, size=.5) + scale_fill_manual(values=c("#f12400","#3d9ab2")) + theme(legend.position = "none") + ylab("Log-Relative Genome Size") + xlab("") + geom_hline(yintercept = 0, lty=2, color="grey") + scale_x_continuous(breaks = unique(plot_dat_gs$phy_id), labels=unique(plot_dat_gs$phylum), expand=c(0.01,0.01)) + theme(axis.text.x = element_text(angle=45, hjust=1), axis.title.y = element_text(size=12))
...
gc_plot = plot_dat_gc %>%  ggplot(aes(x=plot_pos, y=gene_count, fill=cophy, group=paste(phylum,cophy) )) + geom_boxplot(outlier.shape = NA, position = position_dodge()) +  
  ...
  geom_text(data = pvals_wilcox, aes(y=5200, x=as.numeric(as.factor(phylum)), group=NA, fill=NA, label=stars))
...
xylo_plot = dat_xylo %>% 
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1, size=9), axis.title.y = element_text(size=12)) + xlab("") + ylab("D-Xylose util.\nPrevalence  (%)") + theme(legend.position = "none") + 
  scale_y_continuous(limits=c(0,105)) + 
  scale_x_continuous(breaks = unique(plot_dat_gs$phy_id), labels=unique(plot_dat_gs$phylum), expand=c(0.01,0.01)) + 
  geom_text(data = p_vals_fisher, aes(y=100, x=as.numeric(as.factor(phylum)), fill=NA, label=stars))


panels_def = cowplot::plot_grid(
  panel_d,
cowplot::plot_grid(gc_plot+ theme(axis.text.x = element_blank())  + theme(plot.margin = unit(c(0, 0.5, 0, 1), "cm")), xylo_plot  + theme(plot.margin = unit(c(.1, 0.5, 0, 1), "cm")), ncol=1, align = "v", labels=c("e","f"), rel_heights = c(.4,.6)),
labels=c("d",""))
...
bifido_plot = plot_grid(NULL,
                        bifido_marker + geom_text(data=data.frame(x=0,y=12.5,label="GTDB\nMarkers"), hjust=0, aes(label=label)) , 
                        labelplot,  conn2,
                        bifido_gyrb + geom_text(data=data.frame(x=0,y=12.5,label="gyrB"), hjust=1, aes(label=label)), NULL,
                        ncol=6,
...
plot5=plot_grid(
  cophy_tree_set, NULL,
  plot_grid(
    plot_grid(top_right[[1]],NULL, top_right[[2]], labels=c("b","c"), ncol=3, rel_widths = c(.55,.025,.45)
    ),
    plot_grid(panels_def),
    bifido_plot, ncol=1, rel_heights = c(.35,.35,.25), labels=c("","", "g")),
  rel_widths = c(.3,.02,.7), labels=c("a","",""), ncol=3
)
