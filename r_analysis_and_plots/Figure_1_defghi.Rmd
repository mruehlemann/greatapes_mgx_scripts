...
write.table(cands_res %>% select(ogs, tax, B_prev, V_prev, pval, effect, Zscore) %>% left_join(KO_NAMES, by=c("ogs"="ko")), "figures_and_tables/Suppl_Table_S7_Results_EUR_Family_Func.tsv", row.names = F, sep="\t")
...
write.table(cands_meta, "figures_and_tables/Suppl_Table_S8_Results_Human.EUR_Meta_Func.tsv", row.names = F, sep="\t")
