...
## styles
colset=data.frame(
  host=c("HsA1","HsA2","HsD","HsG","Ggorilla","Gberingei","Pttrog","Ptschw","Ptver","Ppan","Pttrog2","Ggorilla2"),
  host_long=c("Homo sapiens (CIV)","Homo sapiens (DRC)","Homo sapiens (DK)","Homo sapiens (GER)","Gorilla gorilla gorilla (GAB)","Gorilla beringei beringei","Pan troglodytes troglodytes (GAB)","Pan troglodytes schweinfurthii","Pan troglodytes verus","Pan paniscus","Pan troglodytes troglodytes (CG)","Gorilla gorilla gorilla (CG)"),
 host_tree=c("Homo_sapiens","Homo_sapiens","Homo_sapiens","Homo_sapiens","Gorilla_gorilla_gorilla","Gorilla_beringei","Pan_troglodytes_troglodytes","Pan_troglodytes_schweinfurthii","Pan_troglodytes_verus","Pan_paniscus","Pan_troglodytes_troglodytes","Gorilla_gorilla_gorilla"),
 col=c("lightblue","lightblue3","deepskyblue4","darkblue","#29AB87","#0B6623","#fecc5c","#fd8d3c","#f03b20","#bd0026","#fecc5c","#29AB87" ),
 shape=c(1,2,3,4,1,2,1,2,3,4,5,5), 
 gid=c(1,2,3,4,1,1.5,2,2.5,3,3.5,2,1), 
 location=c("CIV","DRC","DK","GER","GAB","UGA","GAB","UGA","CIV","DRC","CG","CG"),stringsAsFactors=F)

## sample meta data
sample_meta = read.table("../../../220303_Analysis_New/data_w_dk/groupings_nozoo_campbell_dk.txt", head=F, stringsAsFactors = F)
...
all_genomes = read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.all_genomes.tsv", head=T, sep="\t") %>% mutate(GreatApes_95_final = final, score=Score)
all_genomes = all_genomes %>% mutate(sample = ifelse(grepl("MGYG|Mana", genome), ifelse(grepl("MGY", genome),"HsUHGG","other") , gsub("_cleanbin_","",gsub("[0-9]+$","", genome)))) %>%
  left_join(sample_meta %>% select(sample, host, host_genus, host_long))

sgb_reps_full = read.table(file = "../../../220303_Analysis_New/data_w_dk/GreatApes.representatives_list")
...
genome_sizes = read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.reps_genome_sizes.tsv",head=F, stringsAsFactors = F) %>% select(sgb=V1, genome_size=V2)
gene_count = read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.reps_gene_count.tsv",head=F, stringsAsFactors = F) %>% mutate(sgb = gsub(".faa","", V2), genes_k=V1/1000) %>% select(sgb, genes_k)

sgb_reps_full = sgb_reps_full %>% left_join(genome_sizes) %>% left_join(gene_count)


## SGB abundance table
sgb_abundance = read.table("../../../220303_Analysis_New/data_w_dk/abundance_tables/GreatApes_final_table_bin.tsv", head=T, sep="\t")
sgb_abundance=sgb_abundance[rownames(sgb_abundance) %in% sample_meta$sample,]

## SGB taxonomy
tax = read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.cluster_final_tax.tsv", head=T, stringsAsFactors = F, sep="\t")
...
sgb_reps_full = sgb_reps_full %>% left_join(all_genomes %>% select(genome, Domain, score, Completeness, Contamination))

sgb_reps_full = sgb_reps_full %>% left_join(all_genomes %>% mutate(sample = gsub("_cleanbin_","",gsub("[0-9]+$","", genome))) %>% left_join(sample_meta) %>% filter(!is.na(host_genus)) %>% group_by(GreatApes_95_final, host_genus) %>% summarise(n=n()) %>% dcast(GreatApes_95_final ~ host_genus, value.var="n", fill=0), by=c("sgb"="GreatApes_95_final")) %>% mutate(host = case_when(Homo > 0 & (Gorilla+Pan) == 0 ~ "Human", Homo == 0 & (Gorilla+Pan) > 0 ~ "NHA", Homo > 0 & (Gorilla+Pan) > 0 ~ "both", TRUE ~ "DB"))  
...
### color scheme for phyla
## introduce "phylum2" for plotting; setting all phyla with <= 10 SGBs to "other"
...
#Reds
reds=c("#ea7d82","#d23027","#8c1a21")

#Blues
blues=c("#a4c3ce","#5faac5","#4988a2","#575b7c", "#2b254d")

#Yellows
yellows=c("#ecb81f","#e3792f")

#Greens
greens=c("#85afa5","#188a75","#2d6049")

phylumcols_df=data.frame(col=c("grey",reds[1],greens[1],greens[3] ,yellows[1],blues[1],blues[3],blues[5],reds[3],yellows[2],reds[2],greens[2]), phylum=sort(unique(sgb_reps$phylum2)))

####

## import tree based on GTDBtk marker genes
tree_full = read.tree("../../../220303_Analysis_New/data_w_dk/GreatApes.bac120+ar53.rooted.tre")
tree2=tree_full %>% keep.tip(sgb_reps$sgb)

### rarefied salmon mappings
salmon_rare=read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.salmon_rarefied.tsv",head=T,stringsAsFactors = F)
...
salmon_stats_long=read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.salmon_stats.tsv", head=F, stringsAsFactors = F)
salmon_stats = salmon_stats_long %>% reshape2::dcast(V1 ~ V2, value.var="V3") %>% mutate(total=round(mapped_reads/(mapping_rate/100))) %>% select(sample=V1, mapped=mapped_reads, total, mapped_rel=mapping_rate)
sample_exclude = salmon_stats %>% filter(mapped_rel < 30 | mapped<1000000) %>% pull(sample) 

### Traitar output
traitar = read.table("../../../220303_Analysis_New/data_w_dk/all_predictions_flat_majority-votes_combined.txt",head=F,stringsAsFactors = F, sep="\t") %>% filter(V4=="phypat+GGL")

#sample_stats = read.table("../../../220303_Analysis_New/data_w_dk/GreatApes.sample_summaries.tsv", head=T, stringsAsFactors = F)
...
this_to_sgb = readRDS("../../../220303_Analysis_New/data_w_dk/protein_catalogs/GreatApes.KEGG_ko_to_sgb.Rds") %>% filter(ogs!="ko")
...
if(file.exists("../../../220303_Analysis_New/data_w_dk/pangenome_all.Rds")){
  pangenome_all = readRDS("../../../220303_Analysis_New/data_w_dk/pangenome_all.Rds")
}else{
  # recovered: found in MAGs reconstructed from host group (=genus)
  pangenome_recovered = this_to_sgb %>% group_by(GreatApes_95_final, ogs) %>% summarize(n=n()) %>% 
    left_join(this_to_sgb %>% select(GreatApes_95_final,host_group) %>% distinct %>% group_by(GreatApes_95_final) %>% summarize(n_hg=n())) %>% mutate(sgb_cons = n/n_hg) %>% ungroup
  # inferred: no MAGs for this hostgroup, reconstructing from MAGs not in this host group
  ...
  # combine recovered and inferred
  ...
  saveRDS(pangenome_all, "../../../220303_Analysis_New/data_w_dk/pangenome_all.Rds")
}
  
host_tree = read.nexus("../../../220303_Analysis_New/cospec_w_manara/host_trees_10kTrees//consensusTree_10kTrees_Primates_Version3_chrono.nex")
host_tree3 = read.nexus("../../../220303_Analysis_New/cospec_w_manara/host_trees_10kTrees//consensusTree_10kTrees_Primates_Version3_chrono_multiHs.nex")

```

## results from cophylo analysis
```{r}
### cospec results data
...
###
all_genomes_final = all_genomes %>% left_join(tax2)# %>% select(genome, magscot_domain = domain, magscot_score = score, cluster_97_final, genome_rep97, GreatApes_95_final) %>% left_join(sgb_reps_full %>% select(-genome), by=c("GreatApes_95_final"="sgb")) %>% arrange(GreatApes_95_final, -magscot_score)
...
### cydA in Prevotella data
 
cyda_tree = read.tree("../../../220303_Analysis_New/data_w_dk/analyses/genetree_ranger.tre")

spe_tree = read.tree("../../../220303_Analysis_New/data_w_dk/analyses/markertree_ranger_w_bl.tre")
spe_tree_nodenames = read.tree("../../../220303_Analysis_New/data_w_dk/analyses/species_tree.nodenames.tre")
spe_tree$node.label = spe_tree_nodenames$node.label

events = read.table("../../../220303_Analysis_New/data_w_dk/analyses/cydA.events.csv",head=F, stringsAsFactors = F, sep=",") %>% 
  group_by(V1,V3,V4) %>% summarise(n=n()) %>% mutate(n_rel=n/1000)

fullset = read.table("../../../220303_Analysis_New/data_w_dk/analyses/analysis_gene_set_complete.tsv", head=T, stringsAsFactors = F) %>% mutate(newid=gsub("_cluster95_","", GreatApes_95_final)) %>% mutate(host_group = case_when(host_group=="P" ~ "Pan", host_group=="H" ~ "Human", host_group == "G" ~ "Gorilla"))

dtl_stats = read_tsv("../../../220303_Analysis_New/data_w_dk/final_stats_single_copy.tsv", col_names=c("gene","in_sgbs","in_sgbs_pan","transfers","losses","transfers_norm","losses_norm")) 

### gyrb trees
gyrb_info = read.table("../../../220303_Analysis_New/data_w_dk//analyses/gyrb/gyrb_prot_to_genome.tsv", head=T, stringsAsFactors = F) %>% left_join(colset) %>% mutate(host_group = substr(host,1,1))
bifido_tax = tax2 %>% filter(genus=="g__Bifidobacterium")
bifido_marker_tree = read.tree("../../../220303_Analysis_New/data_w_dk//analyses/gyrb/f__Bifidobacteriaceae.faa.treefile") %>% ape::root(phy=., outgroup="outgroup",resolve.root=T)
bifido_gyrb_tree= read.tree("../../../220303_Analysis_New/data_w_dk//analyses/gyrb/GreatApes.bifido.gyrb.filt.aln.faa.treefile") %>% ape::root(phy=., outgroup="outgroup",resolve.root=T) 
#bifido_moeller = read.tree("../../../220303_Analysis_New/data_w_dk//analyses/gyrb/GyrB.GreatApes_and_Moeller.aln.fna.treefile") %>% root(outgroup="outgroup", resolve.root=T)
bifido_moeller = read.tree("../../../220303_Analysis_New/data_w_dk//analyses/gyrb/GreatApes.bifido.gyrb.filt+moeller.aln.faa.treefile") %>% root(outgroup="outgroup", resolve.root=T)
...
#save(list = c("cospec_results","colset","sample_meta","all_genomes_final","host_tree","salmon_stats","sample_stats","sample_exclude"), file = "../../../great_apes_shiny_app/data/0_all_preprocessed_app.Robj")
