...
fig2_panel_a=res_gen %>% reshape2::dcast(trait ~ comp, value.var="t.value") %>% left_join(res_gen %>% filter(comp=="human") %>% select(trait, log_fold_diff, q)) %>% 
  ...
  ggplot(aes(x=human, y=hdi_diff)) + geom_point(aes(col=status), size=1.5)  + 
    ...
    ggrepel::geom_label_repel(data = . %>% filter(trait %in% c("g__Bacteroides","g__Akkermansia","g__Alistipes","g__Prevotella","g__Succinivibrio","g__Cryptobacteroides","g__Eubacterium_R","g__Methanobrevibacter_A","g__Coprococcus","g__SIG603","g__Senegalimassilia")), aes(label=trait, col=status), max.overlaps = 20, size=1.8) + 
...
fig2_panel_b_neutral = gen[,plotset %>% filter(group=="neutral") %>% pull(tax), drop=F] %>% 
  ...
  geom_jitter(width=.025, height=0, aes(x=gid2+shift, col=host_long), size=1) + 
...
fig2_panel_b = cowplot::plot_grid(
  cowplot::plot_grid(fig2_panel_b_neutral + theme(axis.text.x = element_blank(), axis.text.y = element_text(size=6), plot.margin=margin(t = 0,b=0, l=2, r=1)) + ylab(""),
                                  NULL,
                                  fig2_panel_b_nha + theme(axis.text.x = element_blank(), axis.text.y = element_text(size=6), plot.margin=margin(t = 0,b=0, l=2, r=1)) + ylab(""),
                                  NULL,
                                  fig2_panel_b_human + theme(axis.text.x = element_blank(), axis.text.y = element_text(size=6), axis.title.y = element_text(size=7), plot.margin=margin(t = 0,b=0, l=2, r=1)) ,
                                  NULL,
                                 fig2_panel_b_hdihi + theme(axis.text.x = element_blank(), axis.text.y = element_text(size=6), plot.margin=margin(t = 0,b=0, l=2, r=1), plot.background = element_rect(fill='transparent')) + ylab("") ,
                                  NULL,
                                  fig2_panel_b_hdilo + theme(plot.margin=margin(t = 0, b=0, l=2, r=1), axis.text = element_text(size=6)) + ylab(""), 
                                                     align="v", axis = "lr", rel_heights = c(.15,-.05,.15,-.05,.15,-.05,.15,-.05,.22), ncol=1) ,
                                  cowplot::plot_grid(
                                    ggplot(data=data.frame(x=1,y=1, label="unchanged"),aes(x=x,y=y,label=label)) + geom_text(col="darkgrey", size=2, angle=270) + theme_nothing(),NULL,
                                    ggplot(data=data.frame(x=1,y=1, label="NHA"),aes(x=x,y=y,label=label)) + geom_text(col="#FF8C00", size=2, angle=270) + theme_nothing(),NULL,
                                    ggplot(data=data.frame(x=1,y=1, label="Human"),aes(x=x,y=y,label=label)) + geom_text(col="blue", size=2, angle=270) + theme_nothing(),NULL,
                                    ggplot(data=data.frame(x=1,y=1, label="Human (EUR)"),aes(x=x,y=y,label=label)) + geom_text(col="darkblue", size=2, angle=270) + theme_nothing(),NULL,
                                    ggplot(data=data.frame(x=1,y=1, label="Human (AFR)              "),aes(x=x,y=y,label=label)) + geom_text(col="lightblue3", size=2, angle=270) + theme_nothing(), 
                                  rel_heights = c(.15,-.05,.15,-.05,.15,-.05,.15,-.05,.22), ncol=1),
                                  rel_widths=c(.95,.05) ,
                                  ncol=2 )
...
fig2_panel_c_final =  fig2_panel_c + theme(legend.position = "top", plot.margin = margin(b=0, t=0), legend.text = element_text(size=7), axis.title = element_text(size=7), axis.text = element_text(size=7)) + guides(color=guide_legend(title="", ncol=3), fill=guide_none())

plot2 = cowplot::plot_grid(
  plot_grid(fig2_panel_a + theme(axis.text = element_text(size=7),axis.title = element_text(size=7)), 
           cowplot::get_legend(fig2_panel_c_final),
           fig2_panel_c_final + theme(legend.position = "none") 
            
            , ncol=1, align="v", axis="lr", labels=c("a","c",""), rel_heights = c(.5,.15,.35)),
  NULL,
  fig2_panel_b , 
  ncol=3, rel_widths=c(0.45,.01,.5), labels=c("","","b"))

plot2

ggsave(filename = "figures_and_tables/Figure_2_new.png", plot2, height=8, width=10, bg="white")
ggsave(filename = "figures_and_tables/Figure_2_pub.pdf", plot2, height=110, width=180, bg="white", units = "mm")
...
fig3_panel_a = res_func %>% reshape2::dcast(trait ~ comp, value.var="t.value") %>% left_join(res_func %>% filter(comp=="human") %>% select(trait, log_fold_diff, q, status)) %>% 
  ...
  ggrepel::geom_label_repel(data = . %>% filter(trait %in% top_trait_func), aes(label=trait), max.overlaps = 20, size=1.8) +  
...
plot3=cowplot::plot_grid(fig3_panel_a + theme(axis.text = element_text(size=5), axis.title = element_text(size=7)), 
                   cowplot::plot_grid(
                     fig3_panel_b + theme(axis.text = element_text(size=5), plot.margin = margin(b=-1), axis.title = element_text(size=6),plot.title = element_text(size=6), strip.text = element_text(size=6,margin = margin(0,0,2,0))), 
                     cowplot::plot_grid(plotlist = cowplot::align_plots(
                       fig3_panel_c + theme(axis.text = element_text(size=5), axis.title = element_text(size=6), plot.margin = margin(t=0, b=0), plot.title = element_text(size=6), strip.text = element_text(size=6,margin = margin(0,0,2,0))), NULL,
                       fig3_panel_d + theme(axis.text.x = element_text(size=5, angle=45, hjust=1), axis.text.y = element_text(size=5), plot.margin = margin(t=0, b=0), axis.title = element_text(size=6), plot.title = element_text(size=6,margin = margin(0,0,2,0))) + ggtitle("K01712 - urocanate hydratase"),
                       align="vh", axis = "btlr"
                       )
                    , labels=c("c","d",""), rel_widths = c(.45,.1,.45), ncol=3), ncol=1, labels=c("b",""))
                   , ncol=2, labels=c("a",""))


ggsave(filename = "figures_and_tables/Figure_3_new.png", plot3, height=5, width=12, bg="white")

ggsave(filename = "figures_and_tables/Figure_3_pub.pdf", plot3, height=75, width=180, bg="white", units = "mm")
